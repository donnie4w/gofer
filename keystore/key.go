...
func RsaEncrypt(msg []byte, publickeypath string) (cipherText []byte, err error) {
	...
	return RsaEncryptByBytes(msg, buf)
}

func RsaEncryptByBytes(msg []byte, publickeyBs []byte) (cipherText []byte, err error) {
	defer _recovr()
	pubDecodeBlock, _ := pem.Decode(publickeyBs)
	if parsePublicKey, e := x509.ParsePKIXPublicKey(pubDecodeBlock.Bytes); e == nil {
...
func RsaDecrypt(cipherText []byte, privatekeypath string) (msg []byte, err error) {
	...
	return RsaDecryptByBytes(cipherText, buf)
}

func RsaDecryptByBytes(cipherText []byte, privateBs []byte) (msg []byte, err error) {
	defer _recovr()
	priDecodeBlock, _ := pem.Decode(privateBs)
	if privateKey, e := x509.ParsePKCS1PrivateKey(priDecodeBlock.Bytes); e == nil {
